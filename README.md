...
아래 흐름은 코드와 자동화 테스트로 관리합니다. 실제 생성에는 별도 GPT-SoVITS 설치, 호환 CUDA/PyTorch 조합, ffmpeg가 필요하므로 이 저장소만으로 생성 성공을 보장하지 않습니다.

구현된 흐름:

- 음성 파일 업로드 또는 기존 파일 경로 입력
- 한국어 전용 Voice DB 샘플 폴더(`voice_db/`)로 성별/연령/유형/파일명 매트릭스 관리
- 3~10초 참조 음성 자르기
...
- backend의 GPT-SoVITS 자동 시작은 로컬 기본 경로만 대상으로 합니다. 기본 허용 API는 `http://127.0.0.1:9100`이며, 외부 API를 사용하려면 서버를 시작할 때 `VOICE_LAB_ALLOWED_API_URLS`에 정확한 주소를 쉼표로 명시해야 합니다.
- `/api/save`는 이 도구가 `generated/` 아래에 만든 유효한 OGG 후보만 `approved/`로 복사합니다. 임의 로컬 파일 경로는 저장 대상으로 허용하지 않습니다.
- `/api/reference`의 전체 요청 본문은 multipart 경계와 폼 필드 오버헤드를 포함해 기본 100 MiB로 제한합니다. 따라서 실제 오디오 파일의 허용 최대 크기는 그보다 조금 작습니다. 서버 운영자가 필요하면 시작 환경에서 `VOICE_LAB_MAX_UPLOAD_BYTES`를 설정할 수 있습니다. 이 제한은 개인용 로컬 backend 경계이며, 인터넷에 공개해도 된다는 뜻은 아닙니다.
