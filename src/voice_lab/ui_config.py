...
QUICK_START_STEPS = [
    ...
    "2. 원하는 느낌을 고르고, 읽힐 대사를 입력해요.",
    "3. 생성된 음성을 들어보고 마음에 드는 것만 저장해요.",
]
DEFAULT_HELP_TEXT = (
    "느낌 버튼은 결과를 정리하기 위한 표시입니다. 실제 말투와 감정은 참조 음성의 에너지, 속도, 억양에 가장 크게 좌우됩니다. "
    "원하는 느낌과 비슷한 음성을 넣으면 결과가 좋아집니다."
)
...
def format_candidate_summary(api_status: str, results: list[dict]) -> str:
    if not results:
        return f"{api_status}\n생성된 후보가 없습니다."
    lines = [api_status, "", "생성 완료. 아래 음성을 들어보고 마음에 드는 것만 저장하세요."]
